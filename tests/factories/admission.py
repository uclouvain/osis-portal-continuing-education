...
def RegistrationDictFactory(person_information, state=ACCEPTED, formation=None):
    registration = {
        ...
        'company_number': factory.Faker('isbn10'),
        'vat_number': factory.Faker('ssn'),
        'national_registry_number': str(factory.Sequence(lambda n: '%d' % n)),
        'id_card_number': str(factory.Sequence(lambda n: '%d' % n)),
        'passport_number': str(factory.Sequence(lambda n: '%d' % n)),
